...
const TIER_LABEL = { self: 'Self-reported', peer: 'Peer-verified', assessed: 'Assessed', proven: 'Ecosystem-proven' };
const TIER_CLASS = { self: 'tag-ink', peer: 'tag-invest', assessed: 'tag-skills', proven: 'tag-success' };
const tierBadge = (tier) => `<span class="tag ${TIER_CLASS[tier] || 'tag-ink'}"><span class="tag-dot"></span>${TIER_LABEL[tier] || 'Self-reported'}</span>`;

// ===== Talent table (Executive Precision pattern) =====
const STATUS_META = { available: ['Available', 'var(--success)'], busy: ['On project', 'var(--warning)'], off: ['Unavailable', 'var(--ink-400)'] };
const talentRow = (t) => {
  const [lbl, color] = STATUS_META[t.status] || STATUS_META.available;
  return `
  <tr data-modal="talentDetail" data-id="${t.id}">
    <td><div style="display:flex;align-items:center;gap:12px">
      <div class="av" style="width:36px;height:36px;background:linear-gradient(135deg,var(--skills),var(--blue-500))">${t.name.split(' ').map(n => n[0]).join('')}</div>
      <div><div style="font-weight:600">${t.name}</div><div style="font-size:11px;color:var(--ink-400);margin-top:1px">${t.role} · ${t.loc}</div></div>
    </div></td>
    <td>${tierBadge(t.tier)}</td>
    <td><span class="status-cell" style="color:${color}"><span class="sd"></span>${lbl}</span></td>
    <td class="mono" style="font-weight:600">${t.rate}</td>
    <td class="mono">${t.fcs}</td>
    <td class="mono" style="color:var(--ink-500)">${t.projects}</td>
    <td style="color:var(--ink-500);white-space:nowrap">${t.last}</td>
  </tr>`;
};
const talentTable = (items) => `
  <div class="table-wrap">
    <div class="table-scroll">
    <table class="data-table">
      <thead><tr><th>Talent profile</th><th>Verification tier</th><th>Status</th><th>Rate</th><th>FCS</th><th>Done</th><th>Last active</th></tr></thead>
      <tbody>${items.map(talentRow).join('')}</tbody>
    </table>
    </div>
    <div class="table-foot">
      <span>Showing <b class="mono" style="color:var(--ink-900)">1–${items.length}</b> of <b class="mono" style="color:var(--ink-900)">4,820</b> talents</span>
      <span class="pg">
        <button data-toast="You're on the first page">‹</button>
        <button class="cur">1</button>
        <button data-toast="Page 2 — next 5 of 4,820 talents">2</button>
        <button data-toast="Page 3 — in the full product this pages through all 4,820">3</button>
        <span class="ell">…</span>
        <button data-toast="Page 964 — the long tail">964</button>
        <button data-toast="Page 2 — next 5 of 4,820 talents">›</button>
      </span>
    </div>
  </div>`;
...
const LIST_SOURCES = {
  ...
  skills: {
    items: () => STATE.talents, render: talentCard, table: talentTable,
...
function subAppPage({ key, glyph, name, tagline, mission, cta, ctaModal, stats, filters, views }) {
  ...
  return `
    ...
    <div class="toolbar">
      ...
      <button class="btn btn-ghost btn-sm" data-toast="Talent directory exported as CSV">↓ Export</button>` : ''}
    </div>

    <div class="list-grid" id="list-wrap" data-key="${key}" data-view="cards">${src.items().map(src.render).join('')}</div>
  `;
}
...
PAGES.home = () => {
  ...
  <div class="page-head">
    ...
    <button class="btn btn-primary" data-modal="newIdea" type="button" style="background-color:var(--navy-600)">+ Submit idea</button>
  </div>
...
PAGES.apps = () => `
  ...
  <div class="subapp-grid">
    ...
    ${largeSubappCard('skills','S','SkillsCanvas','Provable skills, paid receipts.','Build a skill profile verified by completed CollaBoard milestones. Get hired.','4,820 talents · 76% tier-verified')}
  </div>
`;
...
PAGES.profile = () => `
  ...
  <div class="card" style="margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:16px;background:var(--blue-50);border-color:var(--blue-100)">
    <div>
      <div style="font-weight:600;font-size:14px">List another skill in under a minute</div>
      <div style="font-size:12px;color:var(--ink-500);margin-top:2px">Same guided flow that got you activated — pick a discipline, pick what you can do.</div>
    </div>
    <a href="Fixars Onboarding.html" class="btn btn-app-skills btn-sm" style="text-decoration:none;flex-shrink:0">+ List a skill</a>
  </div>

  <div class="split">
    <div>
      <div class="card" style="margin-bottom:16px">
        <h3 class="sec-h">Skills · by verification tier</h3>
        ${[
          ['React','Advanced','94','proven'],
          ['Postgres','Expert','89','proven'],
          ['Mobile money APIs','Intermediate','76','assessed']
        ].map(([s, l, sc, tier]) => `
          <div class="kv">
            <div>
  ...
  <div class="split">
    ...
    <aside>
      ...
      <div class="card">
        ...
        <div class="kv"><span class="k">Mobile app</span><a class="v" href="Fixars Mobile.html" style="font-family:'Inter',sans-serif;color:var(--blue-600)">Open preview →</a></div>
        <div class="kv"><span class="k">Activation flow</span><a class="v" href="Fixars Onboarding.html" style="font-family:'Inter',sans-serif;color:var(--blue-600)">Run again →</a></div>
...
Object.assign(window, { PAGES, LIST_SOURCES, ideaCard, campaignCard, projectCard, talentCard, talentTable, feedCard, emptyState, tierBadge, TIER_LABEL });
