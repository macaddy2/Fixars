...
function wireListFiltering(root) {
  ...
  function renderList() {
    ...
    const view = wrap.dataset.view || 'cards';
    const asTable = view === 'table' && src.table;
    wrap.classList.toggle('list-grid', !asTable);
    wrap.innerHTML = items.length
      ? (asTable ? src.table(items) : items.map(src.render).join(''))
    ...
    wrap.querySelectorAll('[data-toast]').forEach(el => el.addEventListener('click', e => { e.stopPropagation(); toast(el.dataset.toast); }));
  ...
  const viewSeg = root.querySelector('#view-toggle');
  viewSeg?.addEventListener('click', e => {
    const btn = e.target.closest('button[data-view]');
    if (!btn) return;
    wrap.dataset.view = btn.dataset.view;
...
document.querySelectorAll('[data-action="rep-toggle"]').forEach(el => el.addEventListener('click', e => {
  e.stopPropagation();
  const num = document.getElementById('rep-num-topbar');
  if (num) num.style.display = num.style.display === 'none' ? 'inline' : 'none';
}));
